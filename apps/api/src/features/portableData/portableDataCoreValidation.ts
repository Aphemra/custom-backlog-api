...
function parseSavedView(value: unknown, index: number): PortableSavedView {
  ...
  if (builtinKey !== null && !CURRENT_BUILTIN_VIEW_KEYS.has(builtinKey)) {
...
export function parsePortableDataCore(value: unknown): PortableDataExportV2 {
  ...
  if (root.formatVersion !== 2) {
    throw new Error(
      "Portable core validation received an unsupported internal version.",
    );
  }

  const data = requireRecord(root.data, "data");

  requireExactKeys(data, ["libraryGames", "collections", "savedViews"], "data");
  ...
  if (
    builtinKeys.length !== CURRENT_BUILTIN_VIEW_KEYS.size ||
    builtinKeySet.size !== CURRENT_BUILTIN_VIEW_KEYS.size ||
    builtinKeys.some(
      (key) => key === null || !CURRENT_BUILTIN_VIEW_KEYS.has(key),
    )
  ) {
